...
export default function Home() {
  ...
  const handlePostRequest = async () => {
    try{
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/read?itemCode=${itemCode}`, {
      method: 'GET',
      headers: {
  ...
  const handlePurchase = async () => {
    ...
    const confirmPurchase = confirm(`ファイナルアンサー？\n\n 合計金額：${totalAmount}円`);
    ...
    try{
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/purchase`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  return (
    <div className="flex flex-col items-center bg-gray-200 min-h-screen p-8">
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-xl font-bold mb-4">商品コードば入力しちゃらんね。</h1>
